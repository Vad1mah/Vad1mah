...
class Level:
            ...
            for col_index,val in enumerate(row):
                if val != '-1':
                    ...
                    if type == 'fg palms':
                        ...
                        if val == '1':
                            sprite = Palm(tile_size, x, y, '../graphics/terrain/palm_large', 71)
                            
                    if type == 'bg palms':
                        sprite = Palm(tile_size, x, y, '../graphics/terrain/palm_bg', 63)
    ...
    def player_setup(self, layout, change_health):
        for row_index, row in enumerate(layout):
            for col_index,val in enumerate(row):
                ...
                if val == '1':
                    hat_sprite = pygame.image.load('../graphics/character/hat.png').convert_alpha()
                    sprite = StaticTile(tile_size, x, y, hat_sprite)
    ...
    def scroll_x(self):
        ...
        if player_x < screen_width / 6 and direction_x == -1:
            self.world_shift = 8
            player.speed = 0
        elif player_x > screen_width * 5/6 and direction_x == 1:
            self.world_shift = -8
            player.speed = 0
        else:
            self.world_shift = 0
    ...
    def check_win(self):
        if pygame.sprite.spritecollide(self.player.sprite, self.goal, False):
            self.create_overwold(self.current_level, self.new_max_level)
    
    def chech_coin_collisions(self):
